...
class MainWindowController: NSWindowController {
    ...
    lazy var inlineTitlebarVC: InlineTitlebarViewController? = nil
    ...
    func setupTitlebar() {
        ...
        if let inlineTitlebarVC = self.storyboard?.instantiateController(withIdentifier:
            NSStoryboard.SceneIdentifier(rawValue: "inlineTitlebarVC")) as? InlineTitlebarViewController {

            inlineTitlebarVC.layoutAttribute = .trailing

            // layoutAttribute has to be set before added to window
            self.window?.addTitlebarAccessoryViewController(inlineTitlebarVC)
        }

        inlineTitlebarVC = self.window?.titlebarAccessoryViewControllers[0] as? InlineTitlebarViewController

        if let nowPlayingVC = self.storyboard?.instantiateController(withIdentifier:
            NSStoryboard.SceneIdentifier(rawValue: "nowPlayingVC")) as? NowPlayingViewController {
        ...
        nowPlayingVC = self.window?.titlebarAccessoryViewControllers[1] as? NowPlayingViewController

        if let playerControlsVC = self.storyboard?.instantiateController(withIdentifier:
            NSStoryboard.SceneIdentifier(rawValue: "playerControlsVC")) as? PlayerControlsViewController {
        ...
        playerControlsVC = self.window?.titlebarAccessoryViewControllers[2] as? PlayerControlsViewController

        if let progressVC = self.storyboard?.instantiateController(withIdentifier:
            NSStoryboard.SceneIdentifier(rawValue: "progressVC")) as? ProgressViewController {
        ...
        progressVC = self.window?.titlebarAccessoryViewControllers[3] as? ProgressViewController

        //        let nowPlayingVC = storyboard.instantiateController(
        //            withIdentifier: NSStoryboard.SceneIdentifier(rawValue: "NowPlayingVC"))
